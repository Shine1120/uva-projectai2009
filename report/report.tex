...
\begin{document}
	...
	\section{Introduction}
		...
		In section \ref{sec:background}, related work on Haar-features, PCA, SVM, AdaBoost, edge detection and intensity are. The implementation of the AdaBoost algorithm applied on the different techniques are discussed in \ref{sec:implementation}. We discuss our experiments and their results in section \ref{sec:results}. Finally, we conclude in section \ref{sec:conclusion}.
	...
	\section{Implementation}\label{sec:implementation}
		...
		\begin{algorithm}[0]
			\caption{AdaBoost learning best features}
			\begin{algorithmic}[1]
			\medskip
			...
			\STATE {$D_{1(i)}=\frac{1}{n}$, with $i=1,...,n$}
			\FOR {$t=1$ to $T$}
		...
		\begin{algorithm}
			...
			\FOR {$t=1$ to $length(\alpha)$}
				\STATE $p \leftarrow p + \alpha_th_t(x)$
			\ENDFOR
