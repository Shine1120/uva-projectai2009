	...
	for r=1:repetitions	
		...
		for i=1:rounds
			...
			for j=1:2 %LOOP FOR REAR AND FRONT            
				...
				[alpha_weights, best_feature_indexs, all_models] = adaboost(ySegms,xSegms,ImgTrain, T, patterns, labels_train);
				model = struct('model',all_models,'weights',alpha_weights,'best_ids', best_feature_indexs);
			...
			%CLASSIFY THE NEW DATA USING THE STRONG CLASSIFIER_____________              
				if (mod(j,2)==0)                 
					...
					[tp_rear(i),tn_rear(i),error_rear(i),classifier_rear]=eval_bills(model,labels_test,ImgTest,0);
				elseif (mod(j,2)~=0) 
					string_name = ['model_' money_dir sprintf('_front%d.mat', i)];
					save(string_name, 'model');
					[tp_front(i),tn_front(i),error_front(i),classifier_front]=eval_bills(model,labels_test,ImgTest,0);
				end
			%VOTE THE BEST FEATURES TO BUILD THE BEST MODEL EVER___________
				if (i==1 && j==1)
					voted_rear  = zeros(2,size(all_models,2));  
					voted_front = zeros(2,size(all_models,2));  
				end
				if (mod(j,2)==0)
