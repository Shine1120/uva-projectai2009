...
% 	save_patterns(120,85) %generate the patterns
	save_patterns(250,400) %generate the patterns
    % Do the CROSS-VALIDATION loop
    money_dir  = 'neur10'; % 'neur05';  
    fit        = ['../money/wholeplusborder/' money_dir '/fit/'];
    unfit      = ['../money/wholeplusborder/' money_dir '/unfit/'];	
	hold_n_out = 100;
	%define hold-out set 	
    ...
    for i=1:rounds
		...
		test = rest_id(ids(1:slice));
		train  = rest_id(ids(slice+1:end));		
